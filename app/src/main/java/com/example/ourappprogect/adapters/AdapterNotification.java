package com.example.ourappprogect.adapters;
...
public class AdapterNotification extends  RecyclerView.Adapter<AdapterNotification.HolderNotification> {
    ...
    public void onBindViewHolder(@NonNull final HolderNotification holder, int position) {
        ...
        final ModelNotification model = notificationArrayList.get(position);
        String name = model.getsName();
        String notification = model.getNotification();
        String image = model.getsImage();
        final String timestamp = model.getTimestamp();
        String sendlerUid = model.getsUid();
        final String pId = model.getpId();

        Calendar calendar = Calendar.getInstance(Locale.getDefault());
        calendar.setTimeInMillis(Long.parseLong(timestamp));              ////bag
        String pTime = DateFormat.format("dd/MM/yyyy hh:mm  aa",calendar).toString();

        DatabaseReference reference = FirebaseDatabase.getInstance().getReference("Users");
        reference.orderByChild("uid").equalTo(sendlerUid).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot ds: snapshot.getChildren()){
                    ...
                    model.setsName(name);
                    model.setsName(image);
                    model.setsName(email);
                    ...
                    try {
                        ...
                        holder.avatarIv.setImageResource(R.drawable.ic_face);
